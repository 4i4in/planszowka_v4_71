...
void main()
{
	...
	if ((drawing_mode == 0.)	&&	(alpha_texture_color.a > 0.9)) //tile base
		{
	...
	else if ((drawing_mode == 1.)	&&	(alpha_texture_color.a > 0.9))	//tile markers; like mountains fe
		{
			vec4 _take_color = texture2D(color_texture, 
										vec2(	(mod(gl_FragCoord.x + cam_xref,shd_big_sqr_size) /shd_big_sqr_size),
												(mod(gl_FragCoord.y + cam_yref,shd_big_sqr_size) /shd_big_sqr_size)
			...
			if (lines_texture_color.a < 1.)
				{
					gl_FragData[2] = vec4(1.,1.,1.,1.);
				}
			else
				{
					gl_FragData[2] = vec4(lines_texture_color.rgb,alpha_texture_color.a);
				}
		}
	else discard;
}
