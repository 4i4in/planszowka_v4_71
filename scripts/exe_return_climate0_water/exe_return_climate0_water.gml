function exe_return_climate0_water(_map,_this_x,_this_y,_dir,_local_temp)
{
	switch(_dir)
		{
	...
	if _map[_new_x][_new_y][$ "base_name"] == "ocean_rifts"
		{
			var _nxy_climate = _map[_new_x][_new_y][$ "climate"][0];
		}
	else if _map[_new_x][_new_y][$ "base_name"] == "deep_water"
		{
			var _nxy_climate = _map[_new_x][_new_y][$ "climate"][0];
		}
	else if _map[_new_x][_new_y][$ "base_name"] == "shallow_water"
		{
			var _nxy_climate = _map[_new_x][_new_y][$ "climate"][0];
		}
	else
		{
			var _nxy_climate = _local_temp;
		}
	return(_nxy_climate);
}